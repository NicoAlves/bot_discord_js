...
function requestTo(msg) {
    ...
    let dataVerites = [];
    let i = 0;
    let verites = db.collection(`${pathBdd}`).doc(msg.guild.id).collection(`${pathBdd}`);
    let query = verites.where('guildId', '==', msg.guild.id).get()
        .then(snapshot => {
            if (snapshot.empty) {
            ...
            let max = dataVerites.length - 1;
            let nb = Math.floor(Math.random() * (max - 0) + 0);
            let verite = dataVerites[nb].content;
            let embed = new Discord.MessageEmbed()
                ...
                .setTimestamp()
                .addField(`${toUser.username}#${toUser.id}`, `${verite}`, false)
